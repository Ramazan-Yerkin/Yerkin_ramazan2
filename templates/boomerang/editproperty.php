                                    ...
                                    <div id="content" class="container">
                                        ...
                                        <div class="block-content block-content-small-padding">
                                            <div class="row">
                                                <div class="col-sm-12">
                                                    <div class="row-fluid">
                                                        <div class="span8 panel panel-default panel-sidebar-1 affix-parent">
                                                            <div class="panel-heading"><h2>{lang_Propertydata}</h2></div>
                                                            <div class="property_content box panel-body">
...
<?php echo form_input('address', set_value('address', $estate['address']), 'class="form-control" id="inputAddress" placeholder="' . lang('Address') . '"') ?>
                                                                ...
                                                                <div role="tabpanel" style="margin-bottom: 0px;" class="tabbable">
                                                                    <ul class="nav nav-tabs language_tabs lang-tabs" data-spy="affix" data-offset-top="650" role="tablist">
<?php $i = 0;
...
foreach ($this->option_m->languages as $key => $val):$i++; ?>
                                                                            ...
                                                                            <?php if (config_db_item('slug_enabled') === TRUE): ?>
                                                                                    <div class="control-group form-group hidden">
                                                                                        <label class="control-label"><?php echo lang_check('URI slug') ?></label>
                                                                                        <div class="controls">
...
<?php echo form_close() ?>
                                                        ...
                                                        <div class="span4 span-default panel panel-default panel-sidebar-1">
            ...
            if(index == '<?php echo $d_selected_index; ?>')
            {
                ...
                parent_elem.find("<?php echo $generate_selector; ?>").not('.skip-input').each( function() {
                    ...
                    if($(this).hasClass('tree-input-value')) {
                        $(this).closest('.form-group,.control-group').find('select').each(function(){
                            if($(this).attr('required')) {
                                $(this).removeAttr('required')
                                $(this).attr('data-required', 'true')
                            }
                ...
                parent_elem.find("<?php echo $generate_selector; ?>").closest('.form-group,.control-group,h5').addClass('hide');
                
                parent_elem.find("<?php echo $generate_selector; ?>").closest('.form-group,.control-group,h5').each( function() {
                    var curr_elem = $(this);
            ...
            $.getJSON( "<?php echo site_url('privateapi/get_level_values_select'); ?>/"+s_lang_id+"/"+s_field_id+"/"+s_value+"/"+parseInt(s_level+1), function( data ) {
                ...
                if(autoselect_next)
                {
                    if(s_values_splited[s_level+1] != '')
                    {
                        ...
                        if(!find_selected.length && s_values_splited[s_level+1].indexOf('&')>=0) {
                            s_values_splited[s_level+1] =  s_values_splited[s_level+1].replace(/&/g, "&amp;");
                            find_selected = select_element.find('option').filter(function () { return $(this).html() == s_values_splited[s_level+1]; });
                        }
