...
if(!isset(${"estate_data_option_" . $field_id})){
    return false;
}

if(!empty(${"estate_data_option_" . $field_id}) && is_numeric(${"estate_data_option_" . $field_id})){
    $rep_id = ${"estate_data_option_" . $field_id};
...
if(sw_count($file_rep) == 0)
{
    ...
    {
        $def_lang_rep_id = $this->option_m->get_property_value($def_lang_id, $estate_data_id, $field_id);
...
if(sw_count($file_rep))
{
    ...
    echo '<h2 class="title">'.${"options_name_".$field_id}.'</h2>';
    echo '</div>';
    ...
    foreach($file_rep as $file_r)
    {
        if(file_exists(FCPATH.'/files/thumbnail/'.$file_r->filename))
        {
            ...
            '    </a>'.
            '    <div class="preview-img"><img src="'._simg('files/thumbnail/'.$file_r->filename,'215x150').'" data-src="'.base_url('files/'.$file_r->filename).'" alt="'.$file_r->filename.'" class="" /></div>'.
            '</li>';
        }
